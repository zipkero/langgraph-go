...
- [x] task-001: database 클라이언트와 DB 접근 도구
...
- [x] task-002: search 클라이언트와 웹검색·본문로딩 도구
...
- [x] task-003: storage 클라이언트와 파일 조작 도구
...
- [x] task-004: config MCP·에이전트 어셈블리 함수
...
- [x] task-005: mcp HTTP 스트리밍 전송(클라이언트·서버)
...
- [x] task-006: vectorstore SupabaseVectorStore
...
- [x] task-007: README 드리프트 정정 및 신규 구현 반영
