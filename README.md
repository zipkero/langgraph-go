...
├── vectorstore    벡터스토어(인메모리/Supabase-pgvector), 유사도 검색, retriever(Chroma 로컬 백엔드는 제외, §16)
...
> 대응 API가 없어 Ollama(로컬 서버, 기본 모델 `nomic-embed-text`)를 사용한다. `InitChatModel`의 스펙 형식
> (`provider:model`, 예: `anthropic:claude-opus-4-8`)은 다른 프로바이더로 확장할 자리만 남겨둔다(현 단계에서
> OpenAI 챗·gemini는 구현 범위 밖). 임베딩 팩토리(`InitEmbeddings`/`EmbeddingClient`)는 Phase 1에서 구현하지 않고,
> 첫 소비처인 `vectorstore`와 함께 Phase 3에서 구현한다(§26).
...
벡터 저장·검색과 retriever 생성을 담당한다. 인메모리/외부(Supabase-pgvector) 백엔드를 제공한다.

> 로컬 영속 백엔드(Chroma)는 제외 범위로 확정되어 구현하지 않는다. `persist_directory`가 Chroma(파이썬·sqlite)
> 내부 포맷이라 Go에서 직접 읽기 어렵고(§28 참조), 현재 경계 테스트가 관련 외부 SDK 의존을 차단한 상태다.
> 인메모리/외부(Supabase-pgvector) 두 백엔드만 구현 대상이다.
>
...
벡터 백엔드는 이 Phase에서 인메모리만 다룬다. Chroma 로컬 영속 백엔드는 제외 범위로 확정되어 구현하지 않는다(§16).
Supabase-pgvector(`match_documents` RPC)는 `database.Client`에 의존하므로 Phase 7로 분리한다(§16). RAG 그래프
...
- `vectorstore` (Chroma, 제외) — `persist_directory` 영속은 Chroma(파이썬·sqlite) 내부 포맷이라 Go에서 직접
  읽기 어렵고, "임베디드 로컬" 뉘앙스(§16)가 Go에선 그대로 성립하지 않는다. 이 난이도를 이유로 로컬 영속
  백엔드는 구현 대상에서 제외했다(§16).
