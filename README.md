...
├── vectorstore    벡터스토어(인메모리/Supabase-pgvector/Chroma 외부 서버), 유사도 검색, retriever(§16)
...
# 아래 §17·§23은 "무엇을 어떻게 조립하는가"의 설계 가이드이지 import 대상 API가 아니다.
└── (다운스트림 구현)
...
패키지는 하위 핵심(`message`/`llm`/`tool`/`graph`/`agent`)에 의존하며, `multiagent`도 `agent`/`graph`/`command` 위의
라이브러리 패키지로 구현되어 있다(§14).

응용 계층(`rag`/`orchestrator`)은 라이브러리 완성 후 다운스트림이 primitive로 직접 구현한다. 이들은 전용 패키지가
따로 있는 게 아니라 `graph`/`command`/`agent`를 조합해 만드는 응용 패턴이라, 라이브러리가 완성형을 고정하면 응용
설계의 자유도가 사라진다. 그래서 의도적으로 분리하고, 이 문서는 그 구현 설계 가이드(§17·§23)를 함께 제공한다.
...
- `Client` (인터페이스)
- `ChatRequest` (Messages, Tools, ToolChoice, Model, Temperature)
...
> 프로바이더 분담: 기본 기준 프로바이더는 OpenAI다. 챗은 `openai:gpt-5.4-nano-2026-03-17`(기본값) 등
> `openai:<model>` 계열, 임베딩은
> `openai:text-embedding-3-small`(1536차원)을 사용해 hanbit-aiagent 교재의 예제를 치환 없이 그대로 따라갈 수
> 있다. 기존 Anthropic(Claude) 챗(예: `anthropic:claude-opus-4-8`)과 Ollama(로컬 서버, 기본 모델
> `nomic-embed-text`) 임베딩은 각각 `InitChatModel("anthropic:<model>")`/`InitEmbeddings("ollama:<model>")`로
> 명시 지정하면 계속 쓸 수 있는 대체 경로다. `InitChatModel`은 `anthropic`/`openai`, `InitEmbeddings`는
> `ollama`/`openai`를 지원하며(`provider:model` 형식, 미지정 spec은 파싱 에러 — 전역 기본값 없음), API 키는
> `WithAPIKey` 옵션이나 각 프로바이더의 환경변수(`ANTHROPIC_API_KEY`/`OPENAI_API_KEY`)로 지정한다.
...
- `NewToolNode(reg tool.Registry, opts ...ToolNodeOption) graph.NodeFunc`
- 마지막 AI 메시지의 `tool_calls`를 이름으로 찾아 실행하고 `ToolMessage`를 상태에 추가한다.
- 도구 Runtime은 호출마다 새로 구성한다 — tool_call ID, 그래프 실행이 ctx에 주입한 RunConfig(§24),
  옵션 `WithToolStore(s)`/`WithToolEventSink(fn)`으로 지정한 store·이벤트 싱크를 전달한다.
- 도구가 반환한 Command는 전파하지 않는다(이 패키지는 graph/command 미참조).
  핸드오프 Command 전파는 `multiagent.HandoffToolNode`(§14) 소관이다.
...
- 옵션: `WithSystemPrompt`, `WithMiddleware`, `WithCheckpointer`, `WithStore`, `WithToolEventSink(fn)`,
  `WithResponseFormat(schema)`, `WithMaxSteps`
- `WithToolEventSink`는 도구 실행 이벤트 싱크를 지정한다(`trace.ToolEventSink()` 직결 가능). 지정 시
  루프가 도구 호출 전(Result nil)/후(Result 채움) 이벤트를 방출하며, 도구 Runtime의 Emit으로도 주입된다.
  도구 Runtime은 호출마다 tool_call ID를 담아 구성된다(도구가 `rt.ToolCallID()`로 자기 호출 식별).
...
- `Checkpointer` (인터페이스)
- `InMemorySaver`
- `Checkpoint` (ThreadID, Values, Next, Metadata, CreatedAt)
...
> **라이브러리 패키지(import 대상).** `Command`/`Send`/`bind_tools`/`create_agent` 위에 수퍼바이저·핸드오프·
> 네트워크·플래너 패턴을 구현해 제공한다. 다른 `rag`/`orchestrator` 응용 계층과 달리, `multiagent`는 전용
> 패키지로 존재해 다운스트림이 그대로 import해서 쓴다.
...
- `HandoffToolNode(reg *tool.Registry) graph.NodeFunc` — tool_calls를 실행하고, 호출 도구 중 `HandoffTool`이
  있으면 그 Command(ToParent/Fanout)를 그래프 엔진에 전파한다(ToolMessage는 Update에 병합). LangGraph
  파이썬의 "도구가 Command를 반환하면 ToolNode가 전파"하는 패턴의 Go 대응 — `prebuilt.ToolNode`는 Command를
  전파하지 않으므로 핸드오프 그래프에서는 이 노드를 쓴다.
...
- `AgentAsNode(a *agent.Agent) graph.NodeFunc` — 에이전트를 그래프 노드로. 그래프 실행이 ctx에 주입한
  RunConfig(§24)를 내부 에이전트에 전달하고, 그래프가 ModeMessages 스트리밍 중이면 내부 에이전트를
  스트림으로 실행해 토큰을 `graph.StreamTokens` 채널로 포워딩한다
- `GraphAsNode(g *graph.Compiled) graph.NodeFunc` — 서브그래프를 그래프 노드로
- `AgentAsTool(a *agent.Agent, name, desc string) tool.Tool` — 에이전트를 도구로. 실행별 설정은
  도구 Runtime의 Config를 내부 에이전트에 전달한다
...
벡터 저장·검색과 retriever 생성을 담당한다. 인메모리/외부(Supabase-pgvector, Chroma) 백엔드를 제공한다.

> Chroma는 `persist_directory`(Chroma 파이썬·sqlite 내부 포맷)를 Go에서 직접 읽는 방식이 아니라, 외부에서
> 실행 중인 Chroma 서버(v2 REST API, 예: 로컬 Docker)를 백엔드로 지정하는 방식으로 지원한다(§28 참조).
> 서드파티 클라이언트(`amikos-tech/chroma-go`)는 경계 테스트가 금지하는 외부 SDK라 쓰지 않고, Ollama 임베딩과
> 같은 `net/http` 직구현으로 add/query를 호출한다. 인메모리/Supabase-pgvector/Chroma 세 백엔드가 구현 대상이다.
>
> Phase 경계: 인메모리 백엔드는 Phase 3에 들어간다. `SupabaseVectorStore`는 `database.Client`(Phase 7)에
> 위임 의존하므로(§19) Phase 7로 분리한다. `ChromaVectorStore`는 textbook-parity 작업에서 추가됐다.

### 타입

- `Store` (인터페이스)
- `InMemoryStore` (로컬 임베딩 검색)
- `SupabaseVectorStore` (pgvector 백엔드, `MatchDocuments`로 `database.Client`에 위임하는 유사도 검색)
- `ChromaVectorStore` (외부 Chroma 서버 백엔드, v2 REST API 직구현)
- `Retriever`
...
- `(SupabaseVectorStore) MatchDocuments(ctx, queryEmbedding []float32, count int) ([]document.Document, error)` —
  질의 임베딩과 유사도가 높은 문서를 `database.Client.MatchDocuments`(`match_documents` RPC)에 위임해 조회하고
  `document.Document`로 변환한다. 이 RPC는 `(query_embedding, match_count)` 시그니처로 메타 필터를 받지 않는다.
  메타데이터 필터링이 필요하면 `database.QueryDocuments`(eq/ilike/gte/lte) 경로를 쓴다
- `NewSupabaseVectorStore(client database.Client, emb llm.EmbeddingClient, opts ...StoreOption) *SupabaseVectorStore` —
  `database` 패키지의 클라이언트와 질의 임베딩용 `llm.EmbeddingClient`를 함께 받는다

### 외부 백엔드 (Chroma)

- `NewChromaVectorStore(baseURL, collectionName string, emb llm.EmbeddingClient) *ChromaVectorStore` — 외부에서
  실행 중인 Chroma 서버(v2 REST API)를 대상으로 컬렉션을 get-or-create 한다. `Add`는 `emb.Embed`로 클라이언트
  측 임베딩 후 add 엔드포인트를, `SimilaritySearch`는 `emb.EmbedQuery` 후 query 엔드포인트를 호출해
  `document.Document`로 변환한다. `AsRetriever`는 다른 백엔드와 동일하게 `Retriever`로 위임한다
...
- `NewSearchClient(apiKey string, opts ...Option) *SearchClient`
...
- `WithRunConfig(ctx, cfg) context.Context` / `RunConfigFromContext(ctx) (RunConfig, bool)` — 그래프
  진입점(`Compiled.Invoke`/`Stream`)이 주입하고, 노드 함수·prebuilt 노드·어댑터가 시그니처 변경 없이
  실행별 설정에 접근하는 통로
...
> 과정에서 추가된 것이다.
>
> `LoadEnv`가 읽는 주요 환경변수: `ANTHROPIC_API_KEY`(챗), `OPENAI_API_KEY`, `TAVILY_API_KEY`,
> `SUPABASE_URL`/`SUPABASE_KEY`. 임베딩은 Ollama(로컬 서버)를 사용하므로 API 키가 필요 없고, `OPENAI_API_KEY`는
> 현재 임베딩 경로에 쓰이지 않는 채로 `Config` 필드·환경변수 로딩만 유지된다.
...
> LangSmith 추적(§25), OpenAI/Anthropic 외 추가 챗 프로바이더(Gemini 등, §4), 도메인 특화 도구(차트/캔버스
> 생성·코드 실행 등 — `tool.FromFunc`로 다운스트림 프로젝트가 직접 정의, §5).

> 라이브러리 vs 응용 계층: 아래 Phase는 **라이브러리(import 대상)** 구현 순서다. `rag`·`orchestrator`는 라이브러리가
> 아니라 다운스트림이 직접 구현하는 응용 계층이므로(§1), 해당 primitive가 준비된 시점 이후의 "(응용)" 구현으로
> 따로 표기한다. `multiagent`는 라이브러리 패키지로 구현되어 있다(§14).
...
(`llm.InitEmbeddings`/`EmbeddingClient`, Ollama)는 Phase 1에서 미뤄 이 Phase에서 구현한다(첫 소비처가 vectorstore라서).
벡터 백엔드는 이 Phase에서 인메모리만 다룬다. Chroma(외부 서버 백엔드)는 이후 textbook-parity 작업에서
추가됐다(§16). Supabase-pgvector(`match_documents` RPC)는 `database.Client`에 의존하므로 Phase 7로 분리한다(§16).
RAG 그래프
(검색→증강→생성, 관련성·환각 평가, 라우팅)는 응용 계층(§17)이라 다운스트림이 이 primitive 위에 직접 구현한다.

### Phase 4 — 멀티에이전트

라이브러리 패키지(§14). Phase 1~2의 primitive(`agent`/`graph`/`command`)가 준비된 뒤 그 위에 구현된다. 수퍼바이저
라우팅(도구 바인딩), 핸드오프(단일/다중 Send), 워커-as-노드/서브그래프, 네트워크 왕복, 플래너(계획/재계획)가
대상이다.
...
라이브러리 토대가 되는 최소 구현 집합(응용 계층 `rag`/`orchestrator`는 제외 — 다운스트림이 이 위에 직접 구현):
...
multiagent:  Supervisor, HandoffTool, AgentAsNode/GraphAsNode/AgentAsTool
```

> 응용 계층은 위 primitive를 조합해 다운스트림이 직접 구현한다(§17·§23이 설계 가이드). 대표 조립 단위:
> `rag`(BuildRAGGraph, GradeRelevance, GradeHallucination), `orchestrator`(AnalyzeIntent, ExecutePlan,
> CallRemoteAgent, GenerateFinalResponse).
...
- `vectorstore` (Chroma) — `persist_directory` 영속은 Chroma(파이썬·sqlite) 내부 포맷이라 Go에서 직접 읽기
  어렵고, "임베디드 로컬" 뉘앙스(§16)가 Go에선 그대로 성립하지 않는다. 그 대신 외부에서 실행 중인 Chroma
  서버를 v2 REST API로 직접 호출하는 백엔드로 지원한다(§16). 서드파티 Go 클라이언트(`amikos-tech/chroma-go`)는
  쓰지 않는다 — `vectorstore/import_boundary_test.go`가 금지 SDK로 명시한다.
