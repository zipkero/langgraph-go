...
> 프로바이더는 OpenAI만 검증 대상이다. 챗·임베딩 모두 OpenAI를 사용하며, `InitChatModel`의 스펙
> 형식(`provider:model`)은 다른 프로바이더로 확장할 자리만 남겨둔다(현 단계에서 anthropic/gemini는 구현 범위 밖).
...
> 과정에서 추가된 것이다.
>
> `LoadEnv`가 읽는 주요 환경변수: `OPENAI_API_KEY`, `TAVILY_API_KEY`, `SUPABASE_URL`/`SUPABASE_KEY`.
...
미들웨어 + 단기 메모리(트리밍·요약) + 구조화 출력)가 동작한다.
...
`document`, `vectorstore`. 적재→분할→임베딩→저장, 유사도 검색, retriever 도구화가 동작한다. 벡터 백엔드는
인메모리 → Chroma(로컬 영속)까지 이 Phase에서 다룬다. Supabase-pgvector(`match_documents` RPC)는 `database.Client`에
